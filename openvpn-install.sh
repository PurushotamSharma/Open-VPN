...
function installOpenVPN() {
	...
	if [[ ! -e /etc/openvpn/server.conf ]]; then
		if [[ $OS =~ (debian|ubuntu) ]]; then
			...
			apt-get install -y openvpn iptables openssl wget ca-certificates curl libpam-google-authenticator qrencode
		elif [[ $OS == 'centos' ]]; then
	...
	if [[ ! -d /etc/openvpn/easy-rsa/ ]]; then
		...
		OPENVPN_PLUGIN_AUTH_PAM_SO_PATH=$(find / -type f -name "openvpn-plugin-auth-pam.so" 2>/dev/null)
	...
	# Check if OPENVPN_PLUGIN_AUTH_PAM_SO_PATH is not empty
	if [ -n "$OPENVPN_PLUGIN_AUTH_PAM_SO_PATH" ]; then
    # Append the plugin directive to the server.conf file
    	echo "plugin $OPENVPN_PLUGIN_AUTH_PAM_SO_PATH \"openvpn login USERNAME password PASSWORD pin OTP\"" >> /etc/openvpn/server.conf
	fi
